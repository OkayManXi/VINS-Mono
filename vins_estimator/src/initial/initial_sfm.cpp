...
bool GlobalSFM::solveFrameByPnP(Matrix3d &R_initial, Vector3d &P_initial, int i, vector<SFMFeature> &sfm_f)
{
    ...
    {
        // choose features those triangulated = 3D points
        if (sfm_f[j].state != true)
            continue;
        Vector2d point2d;
        for (int k = 0; k < (int)sfm_f[j].observation.size(); k++)
        {
            // choose feature points 2d and 3d location those both triangulated and frameid = i
                          ...
                          vector<SFMFeature> &sfm_f, map<int, Vector3d> &sfm_tracked_points)
{
    ...
    */
    ...
    for (int i = 0; i < frame_num; i++)
    {
        ...
        // frame_num-1's t loacation shift not fixed,will be BA
        if (i == l)
        {
    ...
    for (int i = 0; i < feature_num; i++)
    {
        ...
        for (int j = 0; j < int(sfm_f[i].observation.size()); j++)
        {
            ...
            // add cost,set BA output with c_rotation,c_translation and position
