...
{
    trackerData[0].imu_msg_buffer.push_back(
        ImuData(imu_msg->header.stamp.toSec(), imu_msg->angular_velocity.x,
    ...
    if (PUB_THIS_FRAME)
    {
        ...
        if (SHOW_TRACK)
        {
            ...
            for (int i = 0; i < NUM_OF_CAM; i++)
            {
                ...
                {
                    double len = std::min(
                        1.0, 1.0 * trackerData[i].track_cnt[j] / WINDOW_SIZE);
                    ...
                    tmp_prev_un_pts.head(2) = tmp_cur_un_pts - 0.10 *
                    tmp_pts_velocity;
                    tmp_prev_un_pts.z() = 1;
