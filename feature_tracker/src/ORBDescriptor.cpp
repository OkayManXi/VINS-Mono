...
void ORBdescriptor::computeOrbDescriptor(const cv::KeyPoint &kpt, uchar *desc)
{
    ...
    const uchar *center =
        &mBluredImagePyramid.at<uchar>(cvRound(kpt.pt.y * scale) + layer.y,
...
void ORBdescriptor::initializeLayerAndPyramid(const cv::Mat &image)
{
    ...
    int border = std::max(edgeThreshold,
                          std::max(descPatchSize, HARRIS_BLOCK_SIZE / 2)) +
                 1;
    ...
    for (int level = 0; level < nlevels; level++)
    {
        ...
        cv::Size sz(cvRound(image.cols * invscale),
